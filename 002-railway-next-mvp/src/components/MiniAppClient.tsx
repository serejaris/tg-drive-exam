...
export function MiniAppClient() {
  ...
  const [bootModeStarted, setBootModeStarted] = useState(false);
  ...
  useEffect(() => {
    if (authState === "ready" && bootMode && !bootModeStarted) {
      setBootModeStarted(true);
      startSession(bootMode).catch((requestError: Error) => setError(requestError.message));
    }
  }, [authState, bootMode, bootModeStarted, startSession]);
