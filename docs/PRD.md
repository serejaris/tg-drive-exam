...
MVP реализуется в новой изолированной папке `002-railway-next-mvp/` внутри sandbox repo. Нельзя модифицировать существующие попытки `001-*`.
...
- deploy из подпапки `002-railway-next-mvp/` через `railway up --path-as-root`.
